...
class IoULoss(gloss.Loss):
    def __init__(self, weight=None, batch_axis=0, loss_type='giou', x1y1x2y2=False, **kwargs):
        ...
        assert loss_type in ('iou', 'giou', 'diou', 'ciou')
    ...
    def hybrid_forward(self, F, pred, label, sample_weight=None):
        ...
        MAX = 1e5
        EPS = 1e-8
        inter_w = F.clip(F.minimum(b1_xmax, b2_xmax) - F.maximum(b1_xmin, b2_xmin),
                         0, MAX)
        inter_h = F.clip(F.minimum(b1_ymax, b2_ymax) - F.maximum(b1_ymin, b2_ymin),
                         0, MAX)
        ...
        cw = F.maximum(b1_xmax, b2_xmax) - F.minimum(b1_xmin, b2_xmin)
        # convex height
        ch = F.maximum(b1_ymax, b2_ymax) - F.minimum(b1_ymin, b2_ymin)
        ...
        if self.loss_type == 'giou':
            ...
            rho2 = ((b2_xmin + b2_xmax) - (b1_xmin + b1_xmax)) ** 2 \
                   + ((b2_ymin + b2_ymax) - (b1_ymin + b1_ymax)) ** 2
            rho2 = rho2 / 4
            if self.loss_type == 'diou':
                diou = iou - rho2 / c2
                loss = 1. - diou
            elif self.loss_type == 'ciou':
                v = (4 / mx.np.pi ** 2) * (F.arctan(w2 / h2) - F.arctan(w1 / h1)) ** 2
                ...
                raise ValueError(f'unknown loss_type: {self.loss_type}, available: iou, giou, diou, ciou')

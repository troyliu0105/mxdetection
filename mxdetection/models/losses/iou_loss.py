...
class IoULoss(gloss.Loss):
    ...
    def hybrid_forward(self, F, pred, label, sample_weight=None):
        ...
        union = F.elemwise_mul(w1, h1) + F.elemwise_mul(w2, h2) - inter

        iou = F.elemwise_div(inter, union + EPS)  # iou
        ...
        if self.loss_type == 'giou':
            c_area = F.elemwise_mul(cw, ch) + EPS  # convex area
            ...
            rho2 = (((b2_xmin + b2_xmax) - (b1_xmin + b1_xmax) ** 2)
                    + ((b2_ymin + b2_ymax) - (b1_ymin + b1_ymax)) ** 2) / 4
            if self.loss_type == 'diou':
                diou = iou - rho2 / c2
                loss = 1. - diou
            elif self.loss_type == 'ciou':
                v = (4 / mx.np.pi ** 2) * F.power(F.arctan(w2 / h2) - F.arctan(w1 / h1), 2)

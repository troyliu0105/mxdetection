...
def parse_args():
    parser = argparse.ArgumentParser(description='Setup the ImageNet dataset.',
...
def extract_train(target_dir, with_rec=False, num_thread=1):
    os.makedirs(target_dir)
    if with_rec:
        build_rec_process(target_dir, True, num_thread)


def extract_val(target_dir, with_rec=False, num_thread=1):
    os.makedirs(target_dir)
...
def main():
    ...
    extract_train(os.path.join(target_dir, 'train'), build_rec, args.num_thread)
    extract_val(os.path.join(target_dir, 'val'), build_rec, args.num_thread)
